 ...
 */
export const TRUSTED_SITES: AllowedSite[] = [
  {
    domain: 'zerodha.com',
 ...
 */
export function getAllowedSite(url: string): AllowedSite | undefined {
  try {
    const urlObj = new URL(url);
    const hostname = urlObj.hostname.replace(/^www\./, '');
 ...
 */
export function getCSPForSite(url: string): string | null {
