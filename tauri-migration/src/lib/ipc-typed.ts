...
export async function ipcCall<TRequest, TResponse = unknown>(
  ...
  const channelMap: Record<string, string | ((req: any) => string)> = {
    'tabs:list': '/api/tabs',
    'tabs:create': '/api/tabs',
    'tabs:close': (req: any) => `/api/tabs/${req.id}`,
    'tabs:activate': (req: any) => `/api/tabs/${req.id}/activate`,
    'sessions:list': '/api/sessions',
    'sessions:create': '/api/sessions',
    'sessions:getActive': '/api/sessions/active',
    'agent:ask': '/api/agent/ask',
  ...
  try {
    ...
    if (channel.includes(':create') || channel.includes(':start') || channel.includes(':ask')) {
      method = 'POST';
      body = request;
    ...
    // If no mapping found, try generic API client
    if (!endpoint) {
      // Try to use api-client directly
      const apiClientMethods: Record<string, (req?: any) => Promise<any>> = {
        'tabs:list': () => apiClient.tabs.list(),
        'tabs:create': (req: any) => apiClient.tabs.create(req || {}),
        ...
        'system:status': () => apiClient.system.getStatus(),
      };
      ...
      throw new Error(`No HTTP endpoint mapping for channel: ${channel}`);
    }

    // Make HTTP request
    const url = `http://127.0.0.1:4000${endpoint}`;
    ...
    if (IS_DEV) {
      console.warn(`[IPC] HTTP API call failed for ${channel}:`, error);
    }
...
export const ipc = {
  ...
  tabs: {
    create: async (
      ...
      try {
        // Tauri: No need to wait - always use HTTP API
