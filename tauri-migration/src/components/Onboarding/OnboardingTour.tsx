...
export function OnboardingTour({ onClose }: { onClose: () => void }) {
  ...
  // Note: We allow proceeding even without persona selection (will auto-select default)
  const isNextDisabled = false; // Always allow Next - goNext handles validation internally
  ...
  useEffect(() => {
    ...
    const attachListeners = () => {
      ...
      if (nextButton) {
        // Ensure button is enabled (should already be enabled via disabled={false})
                ...
                <label className="flex cursor-pointer items-start gap-3 rounded-2xl border border-slate-700/60 bg-slate-900/70 p-4 hover:border-slate-500/80">
                  <input
                    ...
                    onChange={e => {
                      ...
                      console.log('[Onboarding] Telemetry checkbox changed:', newValue);
                      setTelemetryOptIn(newValue);
                    }}
                    onClick={e => {
                      e.stopPropagation();
                    }}
                    className="mt-1 h-4 w-4 rounded border-slate-600 bg-slate-800 text-emerald-500 focus:ring-2 focus:ring-emerald-500/50 cursor-pointer"
                    style={{
                      pointerEvents: 'auto',
                      zIndex: 10011,
                      position: 'relative',
            ...
            >
              ...
              >
                ...
                <button
                  ...
                  disabled={false} // Never disable - always allow click, goNext handles validation
                  className="rounded-lg border border-emerald-500/60 bg-emerald-500/10 px-4 py-2 font-medium text-emerald-100 transition hover:bg-emerald-500/20 cursor-pointer active:scale-95 focus:outline-none focus:ring-2 focus:ring-emerald-500/50 disabled:opacity-50 disabled:cursor-not-allowed"
                  style={{
                    pointerEvents: 'auto',
                    position: 'relative',
                    zIndex: 10010,
