...
import { runAutomationSequence, AutomationStep } from '../lib/agent/automation';
...
const automationPlaybooks: AutomationPlaybook[] = [
  {
    id: 'zerodha-login',
    label: 'Open Zerodha Login',
    description: 'Launch kite.zerodha.com and focus the User ID field.',
    steps: [
      { action: 'navigate', url: 'https://kite.zerodha.com/' },
      { action: 'wait', value: '3500' },
      { action: 'click', selector: 'input[placeholder="User ID"]' },
    ],
  },
  {
    id: 'kite-dashboard',
    label: 'Go to Kite Dashboard',
    description: 'Navigate to dashboard and capture a quick screenshot.',
    steps: [
      { action: 'navigate', url: 'https://kite.zerodha.com/dashboard' },
  ...
  },
  {
    ...
    description: 'Focus the search box for quick typing.',
    steps: [
      { action: 'navigate', url: 'https://kite.zerodha.com/watchlist' },
      { action: 'wait', value: '4000' },
      { action: 'click', selector: 'input[placeholder="Search eg. infy bse, nifty fut"]' },
    ],
  },
];
...
export function AgentOverlay() {
  const [isActive, setIsActive] = useState(() => {
    ...
    const stored = window.localStorage.getItem('regen:agent-overlay-open');
    return stored !== 'false';
  });
  ...
  useEffect(() => {
    ...
    const handler = (event: Event) => {
      ...
      if (!detail) return;
      const line = `[${new Date().toLocaleTimeString()}] ${detail}`;
      setLogs(prev => [...prev.slice(-199), line]);
    };
    window.addEventListener('agent-log-event', handler as EventListener);
    return () => window.removeEventListener('agent-log-event', handler as EventListener);
  }, []);

  useEffect(() => {
    if (typeof window === 'undefined') return;
    window.localStorage.setItem('regen:agent-overlay-open', isActive ? 'true' : 'false');
  }, [isActive]);
  ...
  const handleAutomationRun = useCallback(
    async (playbook: AutomationPlaybook) => {
      if (isAutomating) return;
      setIsActive(true);
      setActiveTab('logs');
      setIsAutomating(true);
      const startedLine = `[${new Date().toLocaleTimeString()}] ▶ ${playbook.label}`;
      setLogs(prev => [...prev.slice(-199), startedLine]);
      try {
        await runAutomationSequence(playbook.steps);
        toast.success(`${playbook.label} completed`);
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        toast.error(`${playbook.label} failed`);
        setLogs(prev => [
          ...prev.slice(-199),
          `[${new Date().toLocaleTimeString()}] ❌ ${playbook.label}: ${message}`,
        ]);
      } finally {
      ...
      <div className="p-4 border-b border-gray-700/50">
        ...
        <div className="grid grid-cols-3 gap-2 text-xs">
          ...
          <div className="bg-gray-800/50 rounded p-2">
            ...
            {plan?.budget && <div className="text-gray-500 text-xs">/ {plan.budget.seconds}s</div>}
          </div>
          <div className="bg-gray-800/50 rounded p-2">
            <div className="flex items-center gap-1 text-gray-400 mb-1">
              <FileText size={12} />
                ...
                <div className="rounded-lg border border-slate-700/60 bg-slate-900/70 p-3">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm text-slate-200">One-tap Zerodha workflows</p>
                    </div>
                    {isAutomating && (
                      <span className="text-[11px] font-mono text-emerald-300">Running…</span>
                    )}
                  </div>
                  <div className="mt-3 space-y-2">
                    {automationPlaybooks.map(playbook => (
                      <button
                        key={playbook.id}
                        onClick={() => void handleAutomationRun(playbook)}
                        disabled={isAutomating}
                        className="flex w-full items-center justify-between gap-3 rounded-xl border border-slate-800/70 bg-slate-950/40 px-3 py-2 text-left text-sm text-slate-100 transition hover:border-indigo-500/60 disabled:cursor-not-allowed disabled:opacity-60"
                      >
                        <div>
                          <div className="font-semibold">{playbook.label}</div>
