...
export function TabContentSurface({ tab, overlayActive }: TabContentSurfaceProps) {
  ...
  if (!targetUrl) {
    return (
      <motion.div
        ...
        className="h-full w-full overflow-hidden bg-[#0b1120]"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
      ...
      >
        <div className="relative flex h-full w-full flex-col">
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_top,_rgba(56,189,248,0.1),_transparent_55%),_radial-gradient(circle_at_bottom,_rgba(99,102,241,0.08),_transparent_60%)]" />
  ...
  if (isElectron) {
    return (
      ...
      >
        <BrowserTab url={targetUrl ?? undefined} />
      </motion.div>
    );
  }
            ...
            >
              ...
              <div className="flex items-center justify-center gap-2">
                <button
                  type="button"
                  onClick={() => {
                    if (targetUrl && iframeRef.current) {
                      setFailedMessage(null);
                      setLoading(true);
